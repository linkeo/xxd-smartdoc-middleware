      ...
      for (const doc of node.leadingComments) {
        ...
        if (docObject && docObject.type === 'application') {
          ...
          if (docObject.alternativeRoutes && docObject.alternativeRoutes.length > 0) {
            for (let index = 0; index < docObject.alternativeRoutes.length; index++) {
              const route = docObject.alternativeRoutes[index];
              actionObjects.push(_.extend(_.omit(docObject, ['alternativeRoutes']), { route, name: `${docObject.name}-${index + 1}` }));
            }
          }
          for (const actionObject of actionObjects) {
            if (module_) {
              module_.actions = (module_.actions || []).concat(actionObject);
              if (module_.path) {
                actionObject.route.path = path.join(module_.path, actionObject.route.path);
